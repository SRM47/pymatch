...
class TestTensorBase(BaseUnitTest):
    ...
    def test_invalid_non_integer(self):
        ...
        with self.assertRaises(ValueError) as context:
            TensorBase((2, "3", 4))
        self.assertIn("Tensor dimensions must be integers", str(context.exception))

    def test_invalid_list_instead_of_tuple(self):
        """Test TensorBase initialization with a list instead of a tuple."""
        with self.assertRaises(TypeError) as context:
            TensorBase([2, 3, 4])
        self.assertIn("Expected a tuple as input", str(context.exception))
    ...
    def test_invalid_keyword_arguments(self):
        ...
        with self.assertRaises(TypeError) as context:
            TensorBase((2, 3), kw=1)
        self.assertIn(
            "Tensor initialization does not accept keyword arguments",
    ...
    def test_getitem_single(self):
        ...
        with self.subTest(msg="oob_failure"):
            self.assertRaises(
                IndexError, lambda: TensorBase(shape=(2, 3, 4, 5))[2, 0, 0, 0]
            )
