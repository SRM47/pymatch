...
class TestTensorBaseTest(BaseUnitTest):

    def to_tensor(
        self, match_tensor_data: TensorBase, requires_grad=False, get_grad=False
    ) -> torch.Tensor:
        """
    ...
    def randn(self, shape: tuple = None, generator=lambda: gauss(0, 1)):
        if not shape:
            return TensorBase(value=generator())
    ...
    def test_concatenate_default(self):
        ...
        match_tensor = TensorBase(2, 3)
        match_tensor._data = [TensorBase(value=i) for i in range(6)]

        self.assertTrue(
            self.almost_equal(
                TensorBase.concatenate((match_tensor, match_tensor, match_tensor)),
    ...
    #     self.assertTrue(
    #         self.almost_equal(
    #             TensorBase.concatenate((match_tensor, match_tensor, match_tensor), 1),
    ...
    def test_getitem_reference(self):
        ...
        self.assertFalse(
            self.same_references(TensorBase(3, 3, 3)[2, 0, :], tensor[2, 0, :])
        )

    def test_getitem_single(self):
        with self.subTest(msg="normal"):
            self.assertEqual(TensorBase(2, 3, 4, 5)[0, 1, 2, 3].item(), 0)
        with self.subTest(msg="extreme"):
            self.assertEqual(TensorBase(2, 3, 4, 5)[1, 2, 3, 4].item(), 0)
            self.assertEqual(TensorBase(2, 3, 4, 5)[0, 0, 0, 0].item(), 0)
        with self.subTest(msg="oob_failure"):
            self.assertRaises(IndexError, lambda: TensorBase(2, 3, 4, 5)[2, 0, 0, 0])
    ...
    def test_initialize_strides(self):
        self.assertEqual(TensorBase(1, 2, 1)._strides, (2, 1, 1))
        self.assertEqual(TensorBase(6, 4, 2, 5, 7)._strides, (280, 70, 35, 7, 1))
    ...
    def test_single_to_multi_rank_translation(self):
        ...
        with self.subTest(msg="extreme"):
            ...
            self.assertEqual(
                tensor._TensorBase__single_to_multi_rank_translation(59), (3, 2, 4)
            )
        with self.subTest(msg="fail"):
            self.assertRaises(
                IndexError,
                lambda: tensor._TensorBase__single_to_multi_rank_translation(-1),
            )
            self.assertRaises(
                IndexError,
                lambda: tensor._TensorBase__single_to_multi_rank_translation(60),
            )
            self.assertRaises(
                IndexError,
                lambda: tensor._TensorBase__single_to_multi_rank_translation(90),
            )
    ...
    def test_multi_to_single_rank_translation(self):
        ...
        with self.subTest(msg="extreme"):
            ...
            self.assertEqual(
                tensor._TensorBase__multi_to_single_rank_translation((3, 2, 4)), 59
            )
        with self.subTest(msg="fail"):
            self.assertRaises(
                IndexError,
                lambda: tensor._TensorBase__multi_to_single_rank_translation(
                    (-1, 0, -1)
                ),
            )
            self.assertRaises(
